...
class FeedbackRecord extends FirestoreRecord {
  ...
  // "device" field.
  bool? _device;
  bool get device => _device ?? false;
  bool hasDevice() => _device != null;

  void _initializeFields() {
    _comment = snapshotData['comment'] as String?;
    _device = snapshotData['device'] as bool?;
  }
...
class FeedbackRecordDocumentEquality implements Equality<FeedbackRecord> {
  ...
  bool equals(FeedbackRecord? e1, FeedbackRecord? e2) {
    return e1?.comment == e2?.comment && e1?.device == e2?.device;
  }

  @override
  int hash(FeedbackRecord? e) =>
      const ListEquality().hash([e?.comment, e?.device]);
